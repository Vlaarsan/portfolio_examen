          ...
          <div className="grid md:grid-cols-2 gap-6">
            {competencesTechniques.map((c) => (
              <Link key={c.id} to={`/competence/${c.id}`}>
                <div className="p-8 rounded-xl bg-gradient-to-br from-green-500/10 to-blue-600/10 hover:from-green-500/20 hover:to-blue-600/20 transition-all border border-white/10 hover:border-white/30 cursor-pointer h-full">
                  <div className="flex items-start gap-4 mb-3">
                    <i className={`${c.icon} text-3xl text-green-400`}></i>
                    <div className="flex-1">
                      <h2 className="text-2xl font-semibold">{c.nom}</h2>
                      <div className="flex items-center gap-2 mt-1">
                        <StarRating pourcentage={c.pourcentage} />
                        <span className="text-xs opacity-60">{c.niveau}</span>
                      </div>
                    </div>
                  </div>
